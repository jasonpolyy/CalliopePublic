...
from calliope.defaults import MARKETS, FCAS_SERVICES, DEMAND_MIN_HACK, DEMAND_MAX_HACK, MARKET_PRICE_CAP, MARKET_PRICE_FLOOR, PRICE_MIN_HACK, PRICE_MAX_HACK
...
class MeritOrderEnergyNEMEnv(gym.Env):
    ...
    def __init__(
        ...
        action_high = np.array([MAX_BESS_CAPACITY, MAX_BESS_CAPACITY])
    ...
    def step(self, action):
        ...
        # bids_all = [q_energy_bid,q_rreg,q_lreg,q_rf,q_lf,q_rs ,q_ls ,q_rd,q_ld]     
        
        # Create the bid dict
        bess_bid_dict = {
            'ENERGY': [Bid(self.bess_config.station_name, -1000, q_energy_bid, 1)],
            'RAISE6SEC': [Bid(self.bess_config.station_name, 0.0, 0, 1)],
            'RAISE60SEC': [Bid(self.bess_config.station_name, 0.0, 0, 1)],
            'RAISE5MIN': [Bid(self.bess_config.station_name, 0.0, 0, 1)],
            'RAISEREG': [Bid(self.bess_config.station_name, 0.0, 0, 1)],
            'LOWER6SEC': [Bid(self.bess_config.station_name, 0.0, 0, 1)],
            'LOWER60SEC': [Bid(self.bess_config.station_name, 0.0, 0, 1)],
            'LOWER5MIN': [Bid(self.bess_config.station_name, 0.0, 0, 1)],
            'LOWERREG': [Bid(self.bess_config.station_name, 0.0, 0, 1)],
        }
        ...
        delta_soc = eta_c*bess_load - (bess_gen/eta_d)
        ...
        arbitrage = region_price*bess_energy_rescale*((bess_gen_rescale/eta_d)-(eta_c*bess_load_rescale)) 
        ...
        # fcas_revenue = sum([dispatch[service]['price']*get_bess_dispatch(service, rescale=True) for service in FCAS_SERVICES])
        ...
        arbitrage_indicator_reward = bess_energy_rescale*abs_diff_scalar*((discharge_indicator*(bess_gen_rescale/eta_d))+(charge_indicator*eta_c*bess_load_rescale)) 

        # reward 
        reward = arbitrage + arbitrage_indicator_reward + soc_violation #+ fcas_revenue
        ...
        demand_req_obs = [min_max_scale_demand(forecast_demand[f'ENERGY_FC'])]

        price_obs = [min_max_scale_price(dispatch['ENERGY']['price'], cap=1000)]
        ...
        if self.verbose:
            ...
            print(f'Quantity bids: [{bids_all}]')
    ...
    def reset(self, seed=None, options=None):
        ...
        price_obs = [min_max_scale_price(dispatch_init['ENERGY']['price'], cap=1000)]
