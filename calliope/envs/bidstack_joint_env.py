...
class MeritOrderJointNEMEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # charge, discharge, price_bid, rreg, lreg, rf, lf, rs, ls, rd, ld = action#[0]
        ...
        delta_soc = ((eta_c*(bess_load + bess_lreg*self.bess_config.lreg_util)) - ((bess_gen + bess_rreg*self.bess_config.rreg_util)/eta_d))
        ...
        arbitrage = region_price*bess_energy_rescale*((bess_gen_rreg_util_rescale/eta_d)-(eta_c*bess_load_lreg_util_rescale)) 
        ...
        arbitrage_indicator_reward = BETA*bess_energy_rescale*abs_diff_scalar*((discharge_indicator*(bess_gen_rreg_util_rescale/eta_d))+(charge_indicator*eta_c*bess_load_lreg_util_rescale)) 

        # reward 
        reward = arbitrage + arbitrage_indicator_reward + fcas_revenue
