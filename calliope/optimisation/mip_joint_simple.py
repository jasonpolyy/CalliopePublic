...
class MixedIntegerESSModelJoint(AbstractOptimisationModel):
    """
    ...
    def get_bids(self):
        ...
        if not self.is_solved:
            warn("Model has not been solved yet.")
            return
    ...
    def _add_objective(self):
        ...
        for t in self.params.idx:
            ...
            self.objective.SetCoefficient(self.discharge[t], self.params.intervals_per_hour*self.params.ROP[t])
            self.objective.SetCoefficient(self.charge[t] , -self.params.intervals_per_hour*self.params.ROP[t])

            if self.params.AGC is not None:
                self.objective.SetCoefficient(self.rreg[t], self.params.intervals_per_hour*self.params.ROP[t]*self._raise_reg_signal(t))
                self.objective.SetCoefficient(self.lreg[t], -self.params.intervals_per_hour*self.params.ROP[t]*self._lower_reg_signal(t))
        ...
        for t in self.params.idx:
            ...
            if t == 0:
                ...
                if self.init_storage_override is not None:
                    self.solver.Add(self.soc[0] == self.init_storage_override + (1/intervals)*((charge_t*eta_c) - (discharge_t/eta_d)))
                else:
                    self.solver.Add(self.soc[0] == (self.config.init_storage * energy) + (1/intervals)*((charge_t*eta_c) - (discharge_t/eta_d)))
            else:
    ...
    def _cumulative_discharge_constraint(self):
        ...
        for t in self.params.idx:
            if t == 0:
                if self.cumulative_override is not None:
                    self.solver.Add(self.cumulative_discharge[0] == self.cumulative_override)
                else:
